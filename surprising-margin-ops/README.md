...
Margin-operation APIs and providers for liquidation, funding, insurance, and ADL.
...
- `surprising-margin-ops-provider`: combined deployable jar for the four providers above.

## Combined Provider Deployment

`surprising-margin-ops-provider` starts the existing liquidation, funding, insurance, and ADL provider components in one JVM. This is only a packaging merge:

- Business packages remain isolated under `com.surprising.liquidation`, `com.surprising.funding`, `com.surprising.insurance`, and `com.surprising.adl`.
- The four services still coordinate through their existing PostgreSQL tables, Kafka topics, outbox rows, idempotency keys, leases, and sequences.
...
When using the combined provider behind gateway, point all four routes to the same base URL:
