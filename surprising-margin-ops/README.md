...
- 资金费服务启动时从 Instrument 内部聚合 RPC 加载本产品线完整 JVM 快照，费率计算和结算候选使用快照中的
  合约参数与资产精度；运行中消费 `surprising.instrument.events.v1` 增量更新。持仓、余额和结算幂等状态
  仍在同一主库事务内读取和写入。
...
- 风险持久化已经按物理表拆分为账户快照、持仓快照、强平候选、管理规则和 outbox 仓储，由
  ...
  持仓、账户余额、负债和冻结从数据库读取，合约、资产精度和风险档位统一从本 JVM 快照读取，源码已
  标注多表业务查询的“不可拆原因”。
- 强平持久化同样按物理表拆分：候选、持仓锁、强平审计、管理员动作、交易订单、订单事件、
  trading outbox、用户费率分别由独立仓储负责；合约费率、规格、资产精度和风险档位从本 JVM 快照读取。
  `LiquidationService` 与 `LiquidationOrderPersistenceService` 在事务内完成聚合，保证强平订单、订单事件和
  outbox 原子提交。`LiquidationRepository` 只保留业务状态多表一致性查询，并标注“不可拆原因”。
