...
public class RedisAdlCandidateIndex {
    ...
    public void synchronize(RiskPositionUpdatedEvent event) {
        ...
        try {
            if (event.signedQuantitySteps() == 0 || event.unrealizedPnlUnits() <= 0 || event.notionalUnits() <= 0) {
                redis.opsForZSet().remove(key(event.productLine(), event.settleAsset()), member); return;
            }
    ...
    public void synchronize(AdlCandidate candidate) {
        ...
        redis.opsForZSet().add(key(properties.getKafka().getProductLine(), candidate.asset()), member,
                -((double) candidate.priorityScorePpm()));
    }
    public void clear(ProductLine productLine, String asset) { redis.delete(key(productLine, asset)); }
    public void markReady(ProductLine productLine, String asset) {
        redis.opsForValue().set(readyKey(productLine, asset), "1",
                Duration.ofMillis(Math.max(1L, properties.getRedisIndex().getReadyTtlMs())));
    }

    public Optional<List<Member>> candidates(ProductLine productLine, String asset, int limit) {
        try {
            if (!Boolean.TRUE.equals(redis.hasKey(readyKey(productLine, asset)))) return Optional.empty();
            var values = redis.opsForZSet().range(key(productLine, asset), 0, Math.max(0, Math.min(limit, 2_000) - 1));
    ...
    String key(ProductLine productLine, String asset) { return prefix()+":queue:{"+productLine.name()+":"+asset+"}"; }
    String readyKey(ProductLine productLine, String asset) { return prefix()+":ready:"+productLine.name()+":"+asset; }
