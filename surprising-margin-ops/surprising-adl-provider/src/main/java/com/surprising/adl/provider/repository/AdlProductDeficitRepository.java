...
/**
 * 只负责读取 {@code account_product_deficits} 表。
 */
@Repository
public class AdlProductDeficitRepository implements AdlDeficitRepository {

    private final JdbcTemplate jdbcTemplate;

    public AdlProductDeficitRepository(JdbcTemplate jdbcTemplate) {
    ...
    public Map<Long, Long> remainingByUsers(ProductLine productLine, String asset, List<Long> userIds) {
        ...
        jdbcTemplate.query("""
                SELECT user_id, deficit_units - reserved_units AS remaining_units
                  FROM account_product_deficits
                 WHERE account_type = ? AND asset = ? AND user_id IN (%s)
