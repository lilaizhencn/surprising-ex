...
状态：**阶段 7.2a 已完成，阶段 7.2b 待执行**。每个阶段必须完成代码、旧路径清理和正确性验收后才进入下一阶段；压测留到全部核心阶段完成后。
...
- 下一阶段为阶段 7.2b：在不破坏 Owner before-state 和恢复确定性的前提下，迁移批量准入的完成交接；随后按同一模型迁移风控/强平、资金费/结算、ADL、交割和期权等命令，完成后再统一压测。
