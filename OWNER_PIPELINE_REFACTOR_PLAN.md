...
状态：**阶段 8.3 已完成，进入阶段 8.4**。每个阶段必须完成代码、旧路径清理和正确性验收后才进入下一阶段；压测留到全部核心阶段完成后。
...
- 阶段 8.4（触发扫描动态续步）：待完成。把标记价后的触发扫描、OCO 分页和触发子单执行中的动态 mutation/collect 回调改为槽位工作状态，消除扫描期间的临时 lambda 与集合；不得破坏分页 cursor、预算和触发子单提交顺序。
