...
run_linear_flow() {
  ...
  wait_sql_equals "linear close settlement processed" \
    "SELECT count(*) FROM account_processed_trades p JOIN trading_match_trades t ON t.symbol = p.symbol AND t.trade_id = p.trade_id WHERE t.symbol = '${LINEAR_SYMBOL}' AND t.maker_user_id = ${LINEAR_SHORT_USER} AND t.taker_user_id = ${LINEAR_LONG_USER}" \
    "2"
...
run_coin_flow() {
  ...
  wait_sql_equals "coin trades processed by account settlement" \
    "SELECT count(*) FROM account_processed_trades p JOIN trading_match_trades t ON t.symbol = p.symbol AND t.trade_id = p.trade_id WHERE t.symbol = '${COIN_SYMBOL}' AND t.maker_user_id = ${COIN_SHORT_USER} AND t.taker_user_id = ${COIN_LONG_USER}" \
    "2"
