...
TARGET_SYMBOLS_PER_PARTITION="${TARGET_SYMBOLS_PER_PARTITION:-4}"
PROVIDER_INSTANCES="${PROVIDER_INSTANCES:-1}"
STREAM_THREADS="${STREAM_THREADS:-1}"
SINGLE_NODE="${SINGLE_NODE:-true}"
if [[ "${SINGLE_NODE}" == "true" ]]; then
  MIN_PARTITIONS="${MIN_PARTITIONS:-4}"
  MAX_PARTITIONS="${MAX_PARTITIONS:-16}"
  PARTITION_STEP="${PARTITION_STEP:-1}"
...
create_product_topics() {
  ...
  create_topic "${prefix}.leverage.setting.events.v1"
  ...
  create_topic_with_partitions "${prefix}.user.mutations.v1" "${ACCOUNT_COMMAND_PARTITIONS}"
  create_compacted_topic_with_partitions "${prefix}.user.state.changelog.v1" "${ACCOUNT_COMMAND_PARTITIONS}"
