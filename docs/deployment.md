...
- Account position events contain the complete position/collateral snapshot and PostgreSQL revision, are stored in the same transaction as the account mutation, and use `<PRODUCT_LINE>:<userId>` as the Kafka key. Kafka publishing is at-least-once; Redis uses revision CAS and other consumers must tolerate duplicate `eventId` values.
