...
- `account_outbox_events`: account-side Kafka outbox rows, currently used for position update events.
...
- WebSocket private channels: `orders`, `matches`, `positions`.
...
- Liquidation risk re-checks require a fresh `risk_account_snapshots` row. If the latest snapshot is older than `surprising.liquidation.risk.max-snapshot-age` or missing, the candidate is canceled rather than executed.
