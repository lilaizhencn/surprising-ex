...
- Order-provider also maintains a Redis read model for ordinary user open orders. It consumes committed order events, match results, and match trades (the last one updates maker orders), then reloads the authoritative order row before projecting it. Startup uses a token lease and a paged PostgreSQL rebuild in a new generation; an atomic Lua timestamp check prevents a rebuild row from overwriting a newer Kafka update. The generation is made readable only after the scan finishes. `GET /orders/open` uses Redis only while that generation is ready and every referenced row is present; otherwise it falls back to PostgreSQL. This is intentionally a read optimization, not an order-state authority or distributed correctness lock.
- ADL consumes committed risk-position snapshots into one Redis sorted set per settlement asset. Its score is the negative ADL priority, so the highest priority is read first. A token-owned rebuild periodically repopulates every asset from PostgreSQL before setting the readiness marker. ADL never executes from Redis data: each selected member is recalculated against a fresh mark price and locked by `AdlRepository.lockCandidate`; incomplete or unavailable Redis data uses the existing database queue.
