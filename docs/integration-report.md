...
- `JAVA_HOME="$(/usr/libexec/java_home -v 21)" mvn -q -pl :surprising-liquidation-provider -am test`: passed, covering liquidation candidate re-checks, staged sizing, reduce-only preemption, and fail-fast candidate lifecycle updates after match results.
...
| TP/SL trigger orders | `TriggerOrderServiceTest`, `TriggerOrderRepositoryTest`, `MarkPriceTriggerParserTest`, and `TriggerKafkaConfigurationTest` verify TP/SL trigger-condition derivation, scoped `clientTriggerOrderId` idempotency, optional `ocoGroupId` persistence, OCO sibling cancellation in the claim statement, mark-price event parsing, key-preserving Kafka consumer settings, due-row claiming with `FOR UPDATE SKIP LOCKED`, generated reduce-only close order submission through order-provider, rejected generated orders becoming `TRIGGER_FAILED`, and stale `TRIGGERING` reset for retry. |
