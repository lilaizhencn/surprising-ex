...
- `JAVA_HOME="$(/usr/libexec/java_home -v 21)" mvn -q -pl :surprising-trigger-provider,:surprising-gateway-provider,:surprising-order-provider,:surprising-account-provider,:surprising-risk-provider,:surprising-liquidation-provider,:surprising-funding-provider,:surprising-integration-test -am test`: passed, covering TP/SL condition derivation, idempotent client trigger ids, gateway trigger routing, mark-price payload parsing, Kafka consumer replay settings, `FOR UPDATE SKIP LOCKED` trigger claiming, reduce-only generated order submission, and the core Java trading chain.
...
| TP/SL trigger orders | `TriggerOrderServiceTest`, `TriggerOrderRepositoryTest`, `MarkPriceTriggerParserTest`, and `TriggerKafkaConfigurationTest` verify TP/SL trigger-condition derivation, scoped `clientTriggerOrderId` idempotency, mark-price event parsing, key-preserving Kafka consumer settings, due-row claiming with `FOR UPDATE SKIP LOCKED`, generated reduce-only close order submission through order-provider, rejected generated orders becoming `TRIGGER_FAILED`, and stale `TRIGGERING` reset for retry. |
