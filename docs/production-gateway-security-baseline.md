...
- KYC 文档存储必须启用 S3 兼容存储，并在 production profile 中配置 endpoint、bucket、region、access key 和 secret key；代码中的 `us-east-1` 默认值仅服务本地对象构造。
