...
- `JAVA_HOME="${JAVA_HOME:-$(/usr/libexec/java_home -v 21 2>/dev/null || true)}" mvn -q -pl :surprising-index-price-provider,:surprising-mark-price-provider -am -Dtest=IndexPriceCalculatorTest,MarkPriceCalculatorTest,MarkPriceServiceTest -Dsurefire.failIfNoSpecifiedTests=false test`：2026-07-04 通过，覆盖指数有效源不足时返回 `INSUFFICIENT_SOURCES` 且 `indexPrice=null`，mark-price 遇到无价格或不可用状态的 index event 不发布 mark price，并且健康指数价恢复后可以继续发布。
...
- `START_INFRA=false POSTGRES_PORT=55433 PAIR_COUNT=1 LOAD_CONCURRENCY=1 BOOK_DEPTH_LEVELS=2 RUN_FAILURE_SCENARIOS=false BUILD_SERVICES=auto KEEP_TMP=true WS_TIMEOUT=180 MM_REFERENCE_MARKET_ENABLED=false MM_REFERENCE_MARKET_WEBSOCKET_ENABLED=false JAVA_HOME="$(/usr/libexec/java_home -v 21)" ./scripts/full-stack-real-config-smoke.sh`：2026-07-04 通过，日志保留在 `/tmp/surprising-full-stack-real-config.xAFD0M`。该轮复用现有 Docker PostgreSQL/Kafka，provider jar 已是最新所以跳过 Maven package；验证持仓模式切换阻断场景会先真实开仓再放置 pending TP/SL，符合 TP/SL 可平仓位校验；同时覆盖 `LOAD_CONCURRENCY=1` 时并发子进程数组为空的脚本收尾边界。最终捕获 depth events `41`、mark events `175`、funding events `171`，WebSocket/accounting invariants 通过。
- `START_INFRA=false POSTGRES_PORT=55433 PAIR_COUNT=1 LOAD_CONCURRENCY=1 BOOK_DEPTH_LEVELS=2 RUN_FAILURE_SCENARIOS=false BUILD_SERVICES=auto KEEP_TMP=true WS_TIMEOUT=900 FULL_STACK_MARK_PRICE_LISTENER_AUTO_STARTUP=true MM_REFERENCE_MARKET_ENABLED=false MM_REFERENCE_MARKET_WEBSOCKET_ENABLED=false JAVA_HOME="$(/usr/libexec/java_home -v 21)" ./scripts/full-stack-real-config-smoke.sh`：2026-07-04 通过，日志保留在 `/tmp/surprising-full-stack-real-config.ENDSKF`。该轮复用现有 Docker PostgreSQL/Kafka，provider jar 已是最新所以跳过 Maven package；开启 mark-price Kafka listener 后，脚本暂停 `index-price provider` 并注入 `INSUFFICIENT_SOURCES` index event，验证 mark price 停止刷新、普通市价单被拒绝为 `mark price unavailable`、健康指数输入恢复后 mark price 重新发布；随后继续通过 TP/SL、风控、强平、保险基金、ADL、market-maker run-once、WebSocket/accounting invariants。最终捕获 depth events `122`、mark events `421`、funding events `428`。本轮把 full-stack WebSocket 捕获默认窗口提高到 900 秒，避免完整链路耗时超过捕获进程导致误判。
...
- `START_INFRA=false RESET_STATE=true RESET_KAFKA_MODE=recreate START_PROVIDERS=true STOP_PROVIDERS=true BUILD_SERVICES=false KEEP_TMP=true MM_ACCOUNT_COUNT=1 MM_DEPTH_LEVELS=2 MM_REFRESH_LEVELS=1 MM_REFRESH_CYCLES=2 MM_REFRESH_INTERVAL_SECONDS=1 TAKER_ORDER_COUNT=3 LOAD_CONCURRENCY=2 WS_FANOUT_USER_COUNT=3 WS_CAPTURE_TIMEOUT=900 REPORT_FILE=docs/market-maker-fanout-smoke-report.md SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:55433/surprising_exchange JAVA_HOME="${JAVA_HOME:-$(/usr/libexec/java_home -v 21 2>/dev/null || true)}" ./scripts/market-maker-stress.sh`：2026-07-04 通过，没有重新打包 provider jar，日志保留在 `/tmp/surprising-mm-stress.9BkxPs`，报告为 `docs/market-maker-fanout-smoke-report.md`。该干净状态样本复用现有 Docker PostgreSQL/Kafka，启动真实 order/matching/account/websocket/gateway provider，执行 2 轮做市商刷新挂单，6 笔普通用户 taker 订单全部成交并结算，account Kafka 最终 lag 为 `0`；前 3 个 taker 用户分别收到自己的私有 orders/positions WebSocket 事件，脚本会检查私有事件 `userId` 是否串号。
- `JAVA_HOME="${JAVA_HOME:-$(/usr/libexec/java_home -v 21 2>/dev/null || true)}" mvn -q -pl :surprising-market-maker-provider -am -Dtest=QuotePlannerTest,RestReferenceMarketProviderTest,MarketMakerServiceTest,MarketMakerApplicationYamlTest -Dsurefire.failIfNoSpecifiedTests=false test`：2026-07-04 通过，覆盖 market-maker 可选 WebSocket 本地订单簿和 REST 参考盘口兜底。Binance depth stream、OKX books、Bybit V5 orderbook 消息以及 REST 快照可以转换成本地 ticks/steps；QuotePlanner 会按参考盘口每档距离和数量生成本地报价，同时保留 post-only、价格偏离、数量和库存上限保护；该功能在 `application.yml` 中默认关闭。
- `../surprising-ex-web` 中执行 `npm run lint`：2026-07-04 通过，覆盖 Web 交易终端持仓模式切换和条件单管理相关 TypeScript 表面。
- `../surprising-client` 中执行 `flutter analyze` 和 `flutter test`：2026-07-04 通过，覆盖 Flutter 共用 iOS/Android 客户端壳、交易模式到账户类型映射、价格 tick 转换、行情 reducer、移动端持仓模式 UI/API 集成，以及新增的多档 TP/SL 条件单模型、提交面板、开放条件单列表和撤销入口。
...
当前脚本覆盖补充：当 `RESET_STATE=true` 时，`scripts/full-stack-real-config-smoke.sh` 现在也会启动 `surprising-market-maker-provider`，并手动调用私有 `run-once` API。该 smoke 会断言做市商 provider 生成 12 笔 post-only 报价，并验证 provider 生成的报价在 order event 和 matching result 中都保留同一个 traceId。同一个脚本现在也可以通过 `MM_REFERENCE_MARKET_ENABLED=true MM_REFERENCE_MARKET_WEBSOCKET_ENABLED=true` 启用流式参考盘口报价；该模式的长时间真实进程报告仍是生产就绪前的剩余任务。
