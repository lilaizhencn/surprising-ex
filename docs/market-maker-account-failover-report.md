...
- 临时日志目录：`/tmp/surprising-mm-stress.JAOAGZ`

## Provider 拓扑

- Provider nodes：matching=2, account=2, websocket=2, order=1, gateway=1
- Kafka group members：surprising-matching-v1/order.commands=4, surprising-account-v1/match.trades=2
- Extra WebSocket group：surprising-websocket-mm-1782949465941762000-2/depth members=2

## 故障场景

- Account node failure during settlement：stopped account-2 after user trades were written; the account consumer group converged to the surviving node, final Kafka lag reached 0, and the database settled all taker trades.

## 压测参数

- 做市商账号数：4
- 做市商初始深度：每个 symbol、每侧 6 档，每档 50 steps
- 做市商刷新挂单：每个 symbol、每侧 2 档，每档 20 steps
- 普通用户订单数：80
- 普通用户每单数量：2 steps
- 并发度：16
...
- 普通用户 taker 订单：80
- 撮合成交笔数：80
- account 已结算普通用户成交：80
- 客户端并发提交耗时：3302.95 ms
- 客户端提交吞吐：24.22 orders/s
- matching event-time 吞吐：5.32 trades/s
- WebSocket：btc_depth=104 eth_depth=104 private_orders=1 private_positions=1 extra_websocket_depth=104
...
- order accepted -> matching result：80 3150.627 9216.162 17139.738 18367.715 18367.715
- order accepted -> account processed trade：80 4379.938 11587.134 18163.478 19354.352 19354.352

## Account Prometheus 指标

- Prometheus scrape note：account ports unavailable or intentionally stopped: 9186; database settlement count is the authoritative completion check for failover runs.
- match-trade events：processed=0 duplicate=0 failed=0
- processed processing：avg=n/a max=n/a eventLagAvg=n/a eventLagMax=n/a
...
- Kafka lag 采样日志：/tmp/surprising-mm-stress.JAOAGZ/kafka-lag.log
- surprising-account-v1 / surprising.perp.match.trades.v1：maxLag=16 lastLag=0 lastSample=2026-07-01T23:47:33Z
...
- 客户端并发提交耗时包含 32 笔 maker 刷新挂单和 80 笔普通用户 IOC 吃单；吞吐按普通用户 taker 订单数计算。
