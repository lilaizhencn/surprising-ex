...
账户单写者在永续命令事实提交后发布 `account.state.events.v1` 的
`PerpetualAccountStateUpdatedEvent`，携带余额、欠款、持仓、逐仓保证金、逐仓订单冻结和持仓模式的完整
用户快照。risk-provider 直接从这一份快照计算钱包并更新 JVM/Redis 风险组，不再维护第二条风险钱包事件链。
账户状态事件按用户分区键和 `accountRevision` 严格有序，数据库只用于启动恢复、异步投影和审计。
...
`trading_leverage_settings` 按 `user_id + symbol + margin_mode` 保存用户目标杠杆，属于
`LeverageSettingEvent` 的异步投影和启动恢复来源；订单管理入口和下单保证金计算不直接读写该表。
...
`cache_revision` 是分片绝对值的单调修订号。账户模块通过内部快照 RPC 提供启动恢复，并在仓位或
ADL 事务的 outbox 中发布分片 Kafka 增量；订单等模块只在各自 JVM 快照中读取。数据库仍是账户模块
的持久化和最终审计来源，人工修复、紧急导入或灾难恢复后应从 `account_positions` 定期重建核对。
