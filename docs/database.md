...
同一事务还发布 `account.state.events.v1` 的 `PerpetualAccountStateUpdatedEvent`，携带余额、欠款、
持仓、逐仓保证金、逐仓订单冻结和持仓模式的完整用户快照。该事件与风险钱包事件复用同一个
`accountRevision`，用于下游 JVM 快照重建和影子对账；账户版本栅栏完成前，账户数据库事务仍是唯一写入事实源。
