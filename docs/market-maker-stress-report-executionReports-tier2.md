...
- 临时日志目录：`/tmp/surprising-mm-stress.AzsbWt`
...
- Kafka client ids：matching=mm-stress-1783249792810640000-matching-1-0, account=mm-stress-1783249792810640000-account-1-0,mm-stress-1783249792810640000-account-1-1
- Matching owner：symbol=BTC-USDT order.commands.partition=14 clientId=mm-stress-1783249792810640000-matching-1-0
- Matching owner：symbol=ETH-USDT order.commands.partition=17 clientId=mm-stress-1783249792810640000-matching-1-0
...
- 客户端并发提交耗时：44132.127 ms
- 客户端提交吞吐：45.32 orders/s
- matching event-time 吞吐：6.08 trades/s
...
- order accepted -> matching result：2000 1486.892 187580.759 295956.745 310823.787 319799.022
- order accepted -> account processed trade：2000 1816.706 242061.830 387695.905 401206.487 416195.903

## Account Prometheus 指标

- match-trade events：processed=2000 duplicate=0 failed=0
- processed processing：avg=0.37095s max=0.527114s eventLagAvg=49.544056s eventLagMax=113.554208s
...
- Kafka lag 采样日志：/tmp/surprising-mm-stress.AzsbWt/kafka-lag.log
- surprising-account-v1 / surprising.perp.match.trades.v1：maxLag=680 lastLag=0 lastSample=2026-07-05T11:19:40Z

## PostgreSQL 指标

- PostgreSQL delta：commits=30132 rollbacks=0 inserted=41994 updated=78956 deleted=58 fetched=2104774 returned=33301694 blocksRead=22 blocksHit=5098392 hitRatio=100.0% tempFiles=0 tempBytes=0 deadlocks=0
...
- order：httpCount=2128 httpAvg=0.769383s httpMax=0s hikariActive=0 hikariPending=0 hikariMax=60 processCpu=0 systemCpu=0 heap=81.89MiB/4096.0MiB
- matching：httpCount=0 httpAvg=n/a httpMax=n/a hikariActive=0 hikariPending=0 hikariMax=30 processCpu=0 systemCpu=0 heap=90.38MiB/4096.0MiB
- account：httpCount=0 httpAvg=n/a httpMax=n/a hikariActive=0 hikariPending=0 hikariMax=40 processCpu=0.003513 systemCpu=0.395172 heap=59.58MiB/4096.0MiB
- websocket：httpCount=12 httpAvg=0.006996s httpMax=0s hikariActive=n/a hikariPending=n/a hikariMax=n/a processCpu=0 systemCpu=0 heap=50.43MiB/4096.0MiB
- gateway：httpCount=0 httpAvg=n/a httpMax=n/a hikariActive=0 hikariPending=0 hikariMax=30 processCpu=0 systemCpu=0 heap=59.96MiB/4096.0MiB
...
- 本轮 `surprising-account-v1` lag 峰值为 `680`，最终回到 `0`；account 处理 2000 笔成交无 duplicate/failed，但 `order accepted -> account processed trade` 延迟达到 `p50=242061.830ms p95=387695.905ms p99=401206.487ms max=416195.903ms`。这说明当前单节点、account consumer 并发 2 的配置在 2000 taker / 96 并发下可以保持资金正确，但后置结算延迟已经不适合生产峰值。
- 暂不建议用 Redis 承接账户余额或持仓写路径：账户结算需要强事务、幂等键和 ledger 可审计性，Redis 会引入双写和恢复复杂度。更合适的下一步是按 symbol/asset 分区扩大 Kafka partition 和 account consumer 并发、降低单笔 ledger/position SQL 写放大、把 instrument/fee/risk-bracket 等只读配置放入 JVM 进程内缓存，并继续保留 PostgreSQL 作为账户事实源。
