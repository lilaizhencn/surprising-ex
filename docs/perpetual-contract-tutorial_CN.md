...
1. 第一版正式交易只开放单向净持仓 + USDT 结算资产全仓。
2. 第二阶段实现逐仓：增加仓位保证金、追加/减少保证金、逐仓资金费扣减、逐仓强平。
3. 第三阶段开放双向持仓：订单、仓位、风险、强平、资金费全部增加 `positionSide`。
4. 等全仓、逐仓、双向持仓都经过压测和账务审计后，再让用户自由选择。

当前项目更接近第一阶段：按 `userId + settleAsset` 做账户级风险组，适合作为生产系统的基础模式。直接开放双向持仓和逐仓会扩大账务、强平和资金费复杂度，不建议在第一版同时开放。
...
- 逐仓/全仓目前没有完整分开；当前更接近按 `userId + settleAsset` 的账户级风险组。
...
因此，当前项目已经覆盖了“能跑通的核心强平链路”，但还没有完全等同主流交易所的全部清算规则。后续要继续补齐的重点是：逐仓/全仓拆分、破产价/接管价/强平费、事件驱动风控、异常保护模式、极端行情压测和资金安全审计。
