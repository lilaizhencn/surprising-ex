...
当前固定创建 25 个 topic、800 个逻辑 partition；RF=3 后为 2400 个 partition replica，平均每个
Broker 约 800 个，低于 `kafka.m7g.2xlarge` 每 Broker 2000 个 replica 的建议上限，所以首发不需要
5 个 Broker。

这 25 个 Topic 必须全部是 32 分区。特别是 `account.user.commands`、同分区 DLT 和
`account.command.results` 三条 Topic 的分区数必须完全一致。完整清单和部署后校验见
[deployment.md](deployment.md)；已有 symbol-keyed Topic 不允许原地增加分区。
...
终态订单快照会从活动 ZSET/Hash 删除。持仓读模型为防止延迟事件复活旧仓位，会保留零仓位和
revision，因此这里的 100 万持仓 slot 必须按“生命周期内出现过的不同
`productLine + userId + symbol + marginMode + positionSide`”估算，而不是只按当前非零仓位估算。
...
| Matching | `MATCHING_KAFKA_CONCURRENCY=4`、`MATCHING_MAX_POLL_RECORDS=16`、`MATCHING_ENGINE_SHARDS=4`、`MATCHING_RISK_SHARDS=2`；按 poll 批量读取幂等/版本/自成交保护状态并事务处理，同用户/标的冲突逐条复查，symbol 分区内保序 |
...
4. PostgreSQL 高增长表增加 partition、保留期和归档；当前无限增长的普通表不能直接长期运行。
5. 对 Valkey 9.0 做 Lua、Hash/ZSET、revision CAS、持久化恢复和故障切换兼容测试。
6. 增加撮合不可用/恢复中的下单入口保护；当前单撮合恢复期间不能继续无限接受新订单。
