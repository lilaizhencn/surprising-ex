...
本项目里 RocksDB 用于账户/订单事实、撮合状态以及风险计算投影队列。账户余额、仓位、保证金和风险
计算结果的在线事实分别由用户分区 reducer、撮合本地状态和风险 JVM 计算维护；PostgreSQL 只保留
异步读模型、恢复基线和审计记录。
...
因此，当前项目已经覆盖了“能跑通的核心强平链路”、基础逐仓/全仓隔离、强平审计和实际强平费入账，但还没有完全等同主流交易所的全部清算规则。后续要继续补齐的重点是：双向持仓、事件驱动风控、异常保护模式、极端行情压测和资金安全审计。
