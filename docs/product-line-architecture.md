...
当前高性能迁移已完成结果等待器有界化、账户 WAL 批量 group commit、订单号窗口预留、撮合 Outbox 按 stream 分序和批量发布；账户/撮合 Kafka 回调 owner 绑定、订单用户命令按用户分区批次执行、统一 UserMutation 契约与批次分区校验、独立 Book Shard runtime、持久化 symbol-shard 绑定、每 shard checkpoint/outbox 边界、撮合批次 assignment epoch 校验和账户/订单/撮合 Kafka transaction 基础设施已接入。账户和订单用户命令消费者现在会在同一 Kafka 事务内双写产品线隔离的 `user.mutations.v1` canonical 入口，旧 reducer 仍是迁移期执行器。账户和订单 reducer 已发布带 SHA-256 校验和及用户序号的产品线隔离 `user.state.changelog.v1`，并支持按序恢复 checkpoint；撮合 outbox 也会严格按 assignment epoch fencing。统一 User State reducer、完整 Kafka changelog 重建、warm standby 与 Aeron Cluster 仍不是最终完成状态。
