...
- account 结算对 instrument type、spot spec、contract spec、order fee snapshot 等只读元数据做有界 JVM 缓存；跨仓普通负向资金变动在可用余额足够时走 guarded available-balance fast path，避免无必要的 `account_position_margins` 查询/锁。
...
- 可以缓存 instrument/risk bracket/fee schedule/mark snapshot、订单费率快照、强平费上下文等只读或订单生命周期内不变的数据；缓存只能减少读取，不能替代资金账本。
