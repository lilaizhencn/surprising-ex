...
- 临时日志目录：`/tmp/surprising-mm-stress.IdJXMg`
...
- Kafka client ids：matching=mm-stress-1783254644223348000-matching-1-0,mm-stress-1783254644223348000-matching-1-1,mm-stress-1783254644223348000-matching-1-2,mm-stress-1783254644223348000-matching-1-3,mm-stress-1783254644223348000-matching-1-4,mm-stress-1783254644223348000-matching-1-5, account=mm-stress-1783254644223348000-account-1-0,mm-stress-1783254644223348000-account-1-1
- Matching owner：symbol=BTC-USDT order.commands.partition=14 clientId=mm-stress-1783254644223348000-matching-1-4
- Matching owner：symbol=ETH-USDT order.commands.partition=17 clientId=mm-stress-1783254644223348000-matching-1-1
...
- 做市商刷新挂单：每个 symbol、每侧 4 档，每档 10 steps
- 做市商连续刷新轮数：1，轮间隔 1s
- 做市商入口：gateway `/trading/batch`，batchSize=5，makerConcurrency=4
- 普通用户订单数：100
- 普通用户每单数量：1 steps
- 并发度：64
...
- 普通用户 taker 订单：100
- 撮合成交笔数：100
- account 已结算普通用户成交：100
- 客户端并发提交耗时：5256.593 ms
- 客户端提交吞吐：19.02 orders/s
- matching event-time 吞吐：14.32 trades/s
- WebSocket：btc_depth=106 eth_depth=106 private_orders=1 private_positions=1 private_executionReports=3 private_user_0_orders=1 private_user_0_positions=1 private_user_0_executionReports=3 private_user_1_orders=1 private_user_1_positions=1 private_user_1_executionReports=3 private_user_2_orders=1 private_user_2_positions=1 private_user_2_executionReports=3 private_user_3_orders=1 private_user_3_positions=1 private_user_3_executionReports=3 private_user_4_orders=1 private_user_4_positions=1 private_user_4_executionReports=3
...
- order accepted -> matching result：100 312.893 4688.808 6960.198 7098.439 7217.512
- order accepted -> account processed trade：100 1294.234 8278.819 11645.458 11974.059 12012.892

## Account Prometheus 指标

- match-trade events：processed=100 duplicate=0 failed=0
- processed processing：avg=0.213329s max=0.632983s eventLagAvg=3.242477s eventLagMax=5.349004s
...
- Kafka lag 采样日志：/tmp/surprising-mm-stress.IdJXMg/kafka-lag.log
- surprising-account-v1 / surprising.perp.match.trades.v1：maxLag=15 lastLag=0 lastSample=2026-07-05T12:33:05Z

## PostgreSQL 指标

- PostgreSQL delta：commits=1605 rollbacks=0 inserted=2250 updated=3881 deleted=4 fetched=117547 returned=271524 blocksRead=19 blocksHit=182458 hitRatio=99.99% tempFiles=0 tempBytes=0 deadlocks=0
- 表行数：orders=212 orderEvents=212 matchResults=212 matchTrades=100 processedTrades=100 accountLedgers=102 positions=104 openTradingOutbox=0 openAccountOutbox=0

## Provider Prometheus 摘要

- order：httpCount=124 httpAvg=0.850808s httpMax=1.855494s hikariActive=0 hikariPending=0 hikariMax=40 processCpu=0 systemCpu=0 heap=77.86MiB/4096.0MiB
- matching：httpCount=0 httpAvg=n/a httpMax=n/a hikariActive=0 hikariPending=0 hikariMax=20 processCpu=0 systemCpu=0 heap=93.6MiB/4096.0MiB
- account：httpCount=0 httpAvg=n/a httpMax=n/a hikariActive=0 hikariPending=0 hikariMax=30 processCpu=0.009347 systemCpu=0.42962 heap=90.66MiB/4096.0MiB
- websocket：httpCount=7 httpAvg=0.009313s httpMax=0.02725s hikariActive=n/a hikariPending=n/a hikariMax=n/a processCpu=0 systemCpu=0 heap=91.16MiB/4096.0MiB
- gateway：httpCount=0 httpAvg=n/a httpMax=n/a hikariActive=0 hikariPending=0 hikariMax=20 processCpu=0 systemCpu=0 heap=48.65MiB/4096.0MiB
...
- 客户端并发提交耗时包含 32 笔 maker 刷新挂单和 100 笔普通用户 IOC 吃单；吞吐按普通用户 taker 订单数计算。
...
- 本轮用于验证 account instrument metadata 缓存后的真实 provider 启动和结算路径：100/100 taker 成交全部由 account 结算，account lag max `15` 且最终为 `0`，前 5 个私有用户均收到各自 `ORDER_EVENT`、`MATCH_RESULT`、`TRADE` executionReports。
...
- WebSocket 私有事件订阅前 5 个普通用户账号，并逐个断言 orders/positions/executionReports 只推送给对应认证用户；公网生产压测仍应继续把该数值扩大到真实长连接规模。
...
- 订单簿仍有双边深度
- WebSocket 收到 BTC/ETH depth 事件，并且前 5 个普通用户都收到各自的私有 orders/positions/executionReports 事件
