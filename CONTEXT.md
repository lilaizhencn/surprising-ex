...
- **Core Export Event**：为未来历史出口保留的不可变事件协议；当前生产交易路径不编码、不发送，也不写 PostgreSQL。
...
在线交易当前态的唯一权威是 Aeron Cluster 的确定性状态、Cluster Log、Archive 和快照。PostgreSQL 不参与同步交易裁决，
也不在当前交易路径承担历史投影；未来历史出口由独立外围消费者接入，不能回流到 Core owner 热路径。
