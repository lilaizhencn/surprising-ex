...
/** 只负责 {@code trading_order_instrument_lifecycle_fences} 表。 */
@Repository
public class OrderInstrumentLifecycleFenceRepository {

    private final JdbcTemplate jdbcTemplate;

    public OrderInstrumentLifecycleFenceRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public void lockForPlacement(ProductLine productLine, String symbol, Instant now) {
        jdbcTemplate.update("""
                INSERT INTO trading_order_instrument_lifecycle_fences (
        ...
        Boolean blocked = jdbcTemplate.queryForObject("""
                SELECT blocked
                  FROM trading_order_instrument_lifecycle_fences
                 WHERE product_line = ?
                   AND symbol = ?
                   FOR UPDATE
    ...
    public void blockForSettlement(
        ...
        jdbcTemplate.update("""
                ...
                WHERE EXCLUDED.instrument_version >=
                      trading_order_instrument_lifecycle_fences.instrument_version
