...
- The trigger provider consumes only the current product line's mark-price topic and validates Kafka key = payload `symbol`. The Kafka listener only updates an in-memory latest-sample slot; once per second the scheduler converts the persisted mark row to current instrument ticks and claims due rows with PostgreSQL `FOR UPDATE SKIP LOCKED`. Intra-second marks replaced by a newer sequence are intentionally not evaluated.
