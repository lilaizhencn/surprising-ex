...
public class InstrumentSnapshotConsumer {
    ...
    @KafkaListener(
            ...
            containerFactory = "triggerInstrumentSnapshotKafkaListenerContainerFactory")
    public void onInstrumentEvent(ConsumerRecord<String, String> record) {
        try {
            ...
            InstrumentSnapshotSupport.apply(snapshotCache, record.key(), event,
                    properties.getKafka().getProductLine(), "条件单服务");
        } catch (Exception ex) {
            throw new IllegalStateException("条件单合约快照更新失败", ex);
        }
    }
