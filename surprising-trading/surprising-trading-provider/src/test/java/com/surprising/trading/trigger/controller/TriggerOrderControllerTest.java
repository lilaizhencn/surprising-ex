...
class TriggerOrderControllerTest {
    ...
    void mapsPlacementStateRejectionToConflict() {
        ...
        when(service.place(any(PlaceTriggerOrderRequest.class)))
                .thenThrow(new IllegalStateException("trigger placement rejected"));
        TriggerOrderController controller = new TriggerOrderController(service);

        assertStatus(() -> controller.place(request()), HttpStatus.CONFLICT);
    }

    @Test
    void mapsBatchPlacementStateRejectionToConflict() {
        TriggerOrderService service = mock(TriggerOrderService.class);
        when(service.placeBatch(any(BatchPlaceTriggerOrderRequest.class)))
                .thenThrow(new IllegalStateException("trigger batch placement rejected"));
        TriggerOrderController controller = new TriggerOrderController(service);
