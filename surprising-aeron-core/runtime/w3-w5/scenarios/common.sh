...
start_line_subset() {
  ...
  [[ -z "${W4_ACTIVE_LINE:-}" ]] || {
    printf 'ERROR=LINE_ALREADY_ACTIVE line=%s\n' "$W4_ACTIVE_LINE" >&2
    return 2
  }
  ...
  if [[ "${W4_STATIC_ONLY:-false}" == true ]]; then
    printf 'LINE_START=STATIC_PREP productLine=%s runId=%s maker=REQUIRED wallet=ABSENT\n' "$line" "$run_id" >"$W4_LINE_LOG"
    return 0
  fi
  local command_name=up
  [[ "$line" == LINEAR_PERPETUAL ]] || command_name=line-up
  if ! RUN_ID="$run_id" PRODUCT_LINE="$line" WALLET_ENABLED=false TASK_RUN_FRESH="${TASK_RUN_FRESH:-false}" \
      "$W4_RUNNER" "$command_name" >"$W4_LINE_LOG" 2>&1; then
    return 1
  fi
...
stop_line_subset() {
  ...
  if [[ "${W4_STATIC_ONLY:-false}" == true ]]; then
    ...
    if ! RUN_ID="$W4_ACTIVE_RUN_ID" PRODUCT_LINE="$line" WALLET_ENABLED=false TASK_RUN_FRESH="${TASK_RUN_FRESH:-false}" \
        "$W4_RUNNER" down >>"$W4_LINE_LOG" 2>&1; then
      return 1
    fi
