...
readonly PRODUCT_LINE=INVERSE_PERPETUAL
readonly RUN_ID=inverse-perpetual-fixed
readonly TEST_RUN_ID=inverse-perpetual-fixed-test
readonly POSTGRES_PORT=25432
readonly KAFKA_PORT=29092
readonly START_COMMAND=line-up
readonly START_ORDER='postgres,kafka,migrations,core-node0,core-node1,core-node2,exporter,projector,instrument,price,account,command,matching,derivatives-lifecycle,funding,gateway,maker'
...
readonly TEST_ROOT="${TMPDIR:-/tmp}/surprising-product-line-tests/inverse-perpetual"
...
run_test() {
  ...
  PRODUCT_LINE="$PRODUCT_LINE" WALLET_ENABLED=false "$JAVA_HOME/bin/java" \
    ...
    -Dsurprising.aeron.lifecycle-seed=16003 \
