...
readonly TEST_PRODUCT_LINE=LINEAR_PERPETUAL
readonly FIXED_RUN_ID=w4-linear-perpetual-fixed
readonly FIXED_POSTGRES_PORT=25432
readonly FIXED_KAFKA_PORT=29092
readonly SERVICE_ORDER='postgres,kafka,migrations,core-node0,core-node1,core-node2,exporter,projector,instrument,price,account,order,matching,trigger,risk,funding,liquidation,insurance,adl,gateway,maker'
...
case "${1:-test}" in
  start)
    RUN_ID="$FIXED_RUN_ID" PRODUCT_LINE="$TEST_PRODUCT_LINE" WALLET_ENABLED=false TASK_RUN_FRESH=false \
      POSTGRES_PORT="$FIXED_POSTGRES_PORT" KAFKA_PORT="$FIXED_KAFKA_PORT" W4_MAIN_WORKTREE="$MAIN_WORKTREE" "$RUNNER" up
    ;;
  test)
    RUN_ID="$FIXED_RUN_ID" PRODUCT_LINE="$TEST_PRODUCT_LINE" PRODUCT_LINES="$TEST_PRODUCT_LINE" \
