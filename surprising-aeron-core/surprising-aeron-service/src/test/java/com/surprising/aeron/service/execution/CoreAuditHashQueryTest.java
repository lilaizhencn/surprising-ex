...
class CoreAuditHashQueryTest {
    ...
    void explicitGlobalHashObservesCommittedFundsAndSurvivesSnapshot() {
        try (TradingCoreRuntime state = new TradingCoreRuntime(ProductLine.SPOT)) {
            ...
            try (TradingCoreRuntime restored = TradingCoreRuntime.fromSnapshot(ProductLine.SPOT, state.snapshot(100))) {
    ...
    private static long hash(TradingCoreRuntime state, CoreMessageType type) {
