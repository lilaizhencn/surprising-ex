...
public final class RuntimeProjectionState {
    ...
    /**
     * Mutable projection storage is deliberately unordered.  Ordering is a snapshot/export concern;
     * keeping a tree for every state domain made every successful patch pay comparator and entry costs.
     */
    ...
    private final Map<String, Long> fees;
    private final Map<String, Long> insurance;
    private final Map<String, Long> deficits;
    private final Map<String, Long> liquidationFees;
    private final Map<String, Long> fundingResiduals;
    private final Map<String, Long> roundingResiduals;
    private final Map<String, Long> clearingPnl;
    private final Map<String, Long> fundingSettlements;
    private final Map<String, Long> lifecycleSettlements;
    private final Map<String, CoreTreasuryState.FundingProgress> fundingProgress;
    private final Map<String, CoreTreasuryState.LifecycleProgress> lifecycleProgress;
    ...
    private final Map<Long, CoreTriggerOrderState> triggerOrders;
    ...
    RuntimeProjectionState(TradingCoreState initial, long businessStateHash, long fundsStateHash,
        ...
        insurance = new HashMap<>(treasury.insuranceBalances());
        deficits = new HashMap<>(treasury.insuranceDeficits());
        liquidationFees = new HashMap<>(treasury.liquidationFeeBalances());
        fundingResiduals = new HashMap<>(treasury.fundingResidualBalances());
        roundingResiduals = new HashMap<>(treasury.roundingResidualBalances());
        clearingPnl = new HashMap<>(treasury.clearingPnlBalances());
        fundingSettlements = new HashMap<>(treasury.fundingSettlements());
        lifecycleSettlements = new HashMap<>(treasury.lifecycleSettlements());
        fundingProgress = new HashMap<>(treasury.fundingProgress());
        lifecycleProgress = new HashMap<>(treasury.lifecycleProgress());
        leverages = new HashMap<>(initial.leverages());
        algoOrders = new HashMap<>(initial.algoOrders());
        timers = new HashMap<>(initial.cancelAllAfterTimers());
        clientOrders = new HashMap<>(initial.clientOrderIndex());
        triggerOrders = new HashMap<>(initial.triggerOrders());
