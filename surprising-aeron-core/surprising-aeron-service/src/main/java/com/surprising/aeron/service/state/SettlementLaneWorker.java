...
/**
 * Bounded single-producer/single-consumer executor for one Account Lane.
 */
final class SettlementLaneWorker implements AutoCloseable {
    ...
    SettlementLaneWorker(String role, int laneId, int requestedCapacity) {
        if (role == null || role.isBlank() || laneId < 0 || requestedCapacity <= 0) {
    ...
    void execute(Runnable task) {
        ...
        if (waitStrategy == WaitStrategy.BLOCKING && next == consumerSequence) {
