...
public final class PendingClusterIngress {
    ...
    public PendingClusterIngress() { for (int i = 0; i < CAPACITY; i++) entries[i] = new Entry(); }
    /** 单条超限无法通过背压恢复；暂时容量不足则由日志回调推进已有命令后重试。 */
    public boolean hasCapacity(CoreMessage command) {
    ...
    public void add(ClientSession session, CoreMessage command, long timestamp, long position) {
