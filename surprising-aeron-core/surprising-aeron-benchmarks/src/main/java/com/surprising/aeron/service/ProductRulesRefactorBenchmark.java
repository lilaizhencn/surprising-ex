...
public class ProductRulesRefactorBenchmark {
    ...
    @Benchmark
    public long committedProductWorkload(ProductionState state, Counters counters) {
        long result = state.scenario.run();
        if (state.scenario.acceptedOperations() != state.scenario.terminalOperations()
                || state.scenario.acceptedCoreMessages() != state.scenario.terminalCoreMessages()) {
            throw new IllegalStateException("product rule workload left nonterminal commands");
        }
    ...
    public static class ProductionState {
        ...
        public void setUpTrial() {
            ...
            scenario = productLine == ProductLine.SPOT
                    ? SpotMixedWorkload.scenario(
                            SpotMixedWorkload.template(accountLanes, activeUsers, symbols), hftRounds, hftBatchSize)
                    : DerivativeMixedWorkload.scenario(
                            DerivativeMixedWorkload.template(productLine, accountLanes, activeUsers, symbols),
                            hftRounds, hftBatchSize);
        }
