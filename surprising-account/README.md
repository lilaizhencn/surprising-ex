...
- Trading fees use `makerFeeRatePpm` / `takerFeeRatePpm` from the side-specific instrument version. Positive rates debit the user; negative rates credit rebates.
...
- Maker/taker fees from every fill are written to `account_ledger_entries` with `reference_type = TRADE_FEE`.
...
- Trade-fee debits use the same balance/deficit safety path as realized losses. Fee rebates first clear deficits, then increase available balance.
