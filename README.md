...
普通及批量非 reduce-only 下单、撤单支持跨集群消息的有界在途窗口（最多64条消息，每批最多20单）。账户和订单簿使用保守依赖分区，订单身份使用窗口内有界的精确集合，避免20单批次的哈希碰撞导致无关等待；下单账户范围包括潜在成交对手。独立批次可以提前准入并按matcher分片提交，owner仍按日志顺序完成。改单、reduce-only、跨matcher批次、无法完整识别目标的撤单批次、风控、配置和一致性查询保留屏障。每批结算保留自己的原始日志时间和位置；1毫秒复制定时事件收尾（不是延迟上限）。快照前完成在途业务，后台回调不推进交易；实时状态在窗口提交时按最终日志位置组成原子输出批次。批次全部拒绝也必须释放提交队头并唤醒后续准入。

Ordinary and batched non-reduce-only placements and cancellations share a bounded window of 64 messages, with at most 20 items per batch. Conservative account/book partitions include potential placement counterparties; a bounded exact order-ID set avoids false batch conflicts. Independent batches can admit and submit to their matcher shard before earlier completion, while the owner commits in log order. Amendments, reduce-only orders, cross-matcher batches, cancellations without fully identifiable targets, risk/configuration commands and coherent queries remain barriers. Every batch retains its original log timestamp and position. A replicated one-millisecond timer drains partial windows (not a latency guarantee). Snapshots drain pending work; background callbacks never advance trading. Realtime changes form an atomic group at the window's final log position. Fully rejected batches release the submission head and wake later admissions.
...
同一事件在 Lane 内直接完成本命令的 pending reservation，并生成该 sequence 的最终订单/持仓索引值和 primitive 资金增量；
订单 `updatedAt/clusterPosition` 也在该事件的所属 Lane 内一次写入，成交完成后不再二次 fan-out 做 metadata stamping；
owner 只按 sequence 消费 completion、单遍安装 Lane 已生成的终态、合并 sequence-local 资金 accumulator、推进连续水位并发布 Aeron 边界，不再二次扫描终态订单或逐余额、逐 reservation、逐订单、逐持仓重算业务结果。
普通命令的跨 Lane watermark 提交使用可复用 `LaneCommitEvent` 一次 fan-out，并通过 completion bitmap 收敛；不再由 owner
逐 Lane 等待 queue slot。同步响应和 snapshot/query fence 仍在完整 committed watermark 后发布。
...
或 projector 线程。显式 Snapshot/query fence 直接从权威 runtime 物化 immutable image 并复算边界所需 hash；section 编码
在同一确定性 snapshot fence 内完成。生产默认不启动 Core Fact
...
AWS 构建机、发布包与三节点配置准备见 [deployment/aws/README.md](deployment/aws/README.md)。此入口不代表四节点联调或性能验收已完成。
...
当前六条产品线仍各自隔离。期权采用 OKX 非组合保证金（non-PM）的 USDⓈ 结算模型：多头在成交时全额支付权利金，IMR/MMR 为零且不参与普通强平；空头保证金使用指数价、同到期远期价、虚值额、合约标记价和风险档位中的 `optionMarginFactorPpm`。期权不使用用户杠杆设置，Core 会拒绝期权 `UPDATE_LEVERAGE`。

`ApplyMarkPriceCommand`、runtime、业务 hash 与 snapshot v25 同时保存合约 mark、index 和 same-expiry forward ticks。`MarkPriceEvent.sameExpiryForwardPrice` 是明确输入；永续价格计算器的 funding-convergence `price1` 不会再被当作期权远期价，OPTION 缺少该输入时不会发布到 Core。

卖开预冻结使用 `max(position IMR - order premium, minimum open margin)`，成交权利金直接补入空头仓位保证金；买平预冻结会扣除可释放的仓位保证金。cross 风险权益使用带方向的期权市值，展示 UPL 仍按 entry/mark 单独计算。强平按触发 mark 关闭空头现金义务，保险基金不足后进入确定性 ADL；ADL 对手方按 mark 关闭相反盈利仓位并承担指定 covered loss。清算义务通过现有 clearing treasury 现金等价记账完成，不在交易 owner 中保留终态接管仓位。
