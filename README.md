...
- User position, position-list, and position-margin reads use a Redis Hash projection only. PostgreSQL remains authoritative. Account transactions capture one revisioned final snapshot per changed position key and, after commit, offer it directly to a bounded coalescing Redis worker; position-cache snapshots do not use the account outbox or Kafka. Queue overflow or Redis failure marks the product-line cache unavailable and the existing PostgreSQL reconciliation rebuild repairs it. Redis applies snapshots with atomic Lua CAS, and user reads fail closed with HTTP 503 while the cache is unavailable or rebuilding. See [docs/position-redis-cache.md](docs/position-redis-cache.md).
