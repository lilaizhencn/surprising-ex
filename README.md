...
- [Perpetual backend performance and architecture optimization report](docs/performance-architecture-optimization-report_CN.md)
...
- Static TP/SL always uses a Spring Data Redis + Lettuce sorted-set candidate index. Redis narrows price-range candidates; PostgreSQL still performs the exact price/status/expiry/OCO claim and remains the only authoritative state. Normal user open-order queries use a replayable, product-line-isolated Redis projection: each user has a same-slot `orderId` ZSET plus full-snapshot and revision hashes; only `ACCEPTED`/`PARTIALLY_FILLED` orders are cached, while terminal revisions prevent delayed events from restoring removed rows. Opaque `orderId` cursor pagination reads Redis only when its epoch is ready and every snapshot is complete; otherwise it falls back to a product-scoped PostgreSQL keyset query. Cancel, amend, freeze/release, matching, liquidation, and final order state remain PostgreSQL conditional-write and transactional-outbox authority. See [docs/open-order-redis-cache.md](docs/open-order-redis-cache.md).
...
- When a normal close, liquidation fill, delivery settlement, or option exercise reduces a position to zero, account-provider emits a durable full position event only. Trigger-provider consumes it and, in its own transaction, cancels exact-scope pre-event `PENDING` triggers plus their status outbox; order-provider independently prunes invalid reduce-only orders. Account-provider never writes trading order or trigger tables.
...
- User position, position-list, and position-margin reads use a Redis Hash projection only. PostgreSQL remains authoritative. Each account transaction stores one complete revisioned position event in the account outbox with Kafka key `<PRODUCT_LINE>:<userId>`. The exact same snapshot is offered to a bounded coalescing Redis worker after commit for low latency, while a dedicated account position-cache consumer provides durable replay after process exits or Redis outages. Redis applies both paths with atomic Lua CAS; queue/Redis failures remove readiness and user reads fail closed with HTTP 503 until replay or PostgreSQL reconciliation repairs the cache. See [docs/position-redis-cache.md](docs/position-redis-cache.md).
...
- Symbol-partitioned consumers reject records whose Kafka key does not match payload `symbol`. Account user commands and complete position events instead require `<PRODUCT_LINE>:<userId>` so one user's financial state stays ordered.
