...
- When a normal close, liquidation fill, delivery settlement, or option exercise reduces a position to zero, account settlement cancels its remaining `PENDING` trigger orders in the same PostgreSQL transaction. The committed position outbox event then makes trigger-provider remove the corresponding static TP/SL Redis members; `GET /open` immediately follows the authoritative canceled database state.
