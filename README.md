...
- Take-profit, stop-loss, and trailing stop are conditional trigger orders driven by mark price. Trigger-provider keeps only the newest mark event per symbol and evaluates it once per second. Due rows submit an idempotent `reduceOnly=true` close order through order-provider with `clientOrderId=trigger-<triggerOrderId>`. Trailing stop uses integer `callbackRatePpm` (`1000` = `0.1%`, `100000` = `10%`) plus optional `activationPriceTicks` and tracks the sampled post-activation high/low watermark.
