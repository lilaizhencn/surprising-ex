...
- Static TP/SL always uses a Spring Data Redis + Lettuce sorted-set candidate index. Redis narrows price-range candidates; PostgreSQL still performs the exact price/status/expiry/OCO claim and remains the only authoritative state. Normal user open-order queries use a replayable Redis projection after it has been rebuilt; missing, incomplete, or unavailable projection data falls back to PostgreSQL. Cancel, expiry, OCO cancellation, successful execution, rejected execution, and transaction rollback remove or reconcile index members; unavailable/not-ready Redis falls back to the existing database claim path for already committed orders.
