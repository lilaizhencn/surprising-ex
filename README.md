...
- User position, position-list, and position-margin reads use a Redis Hash projection only. PostgreSQL remains authoritative; position and collateral triggers append revisioned `POSITION_CACHE_PROJECTED` account-outbox rows in the same transaction for trade settlement, closes/liquidations, funding, ADL, delivery/exercise, and manual isolated-margin changes. Redis applies the projection with atomic Lua CAS and user reads fail closed with HTTP 503 while the cache is unavailable or rebuilding. See [docs/position-redis-cache.md](docs/position-redis-cache.md).
