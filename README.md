...
普通准入和结算的 `LanePublication` 缓冲随现有事件复用，发布与失败回收清空引用并保留数组容量。已在真实本机单节点 SPOT 链路做短测及 JFR，尚未达到目标吞吐，不能视为完整性能验收；结果统一见 `PERFORMANCE_VALIDATION.md`。

待结算冻结量由各 Lane 按资产维护 primitive 用户计数表；用户完成后删除计数项，资产表容量复用，避免每次交易创建再删除用户小 Map。快照恢复清空这些在途计数；吞吐与内存效果以真实单节点采样为准。

交易账户归所属 Account Lane。异步直接命令的订单提交时间/日志位置，以及已平仓持仓对应触发单的取消，由现有 `LaneCommitEvent` 在 Lane 内写入；Owner 等待全部参与 Lane 完成后发布变更并回收事件。普通撮合仍由 `MatcherSettlementEvent` 在结算时盖章；所属 Lane 在持仓完全归零时取消匹配的待触发保护单，部分平仓保留保护单。

风险续扫和内部触发扫描均不再由 Owner 接管账户；过期、移动止损、OCO取消、claim及子单冻结在所属永久 Lane 执行，Owner按页轮询完成并协调子单顺序。OCO分页传递有界ID范围，不复制sibling列表；持久化游标沿用原风险扫描状态。批量风险续扫的收尾提交复用异步 `LaneCommitEvent`，由所属 Lane 写入订单提交元数据，Owner 仅轮询完成并发布结果。批量初始准备及部分改单/控制路径仍有待迁移的同步账户访问。

异步直接命令失败后，Lane 使用已有回滚前值恢复余额、冻结、订单、持仓和账户控制状态，Owner 再恢复发布索引与全局版本。其他内部控制和部分批量路径的同步账户访问仍在迁移中，不能将当前代码视为已全面消除所有权交接。整改与验证状态见根目录 `PERFORMANCE_AND_LANE_FINDINGS.md`、`PERFORMANCE_VALIDATION.md`。

撤单的流水线依赖按被撤订单的方向和撮合价格判断：同账户、同订单及可能消耗该流动性的命令保持顺序，无关同侧或非交叉命令不再等待整个币对排空。节点退出时打印实际窗口峰值及各类阻塞决策计数；这些计数包含初始化、预热和重新判断，不能当成测量期等待时间或不同请求数。

Lane 准入索引在相同业务范围内原地调整订单剩余量，元数据更新保留现有聚合；终态和范围变化才移除或迁移索引项。这减少了空聚合删除后立即重建的中间状态，尚未证明端到端吞吐提升。

Owner 的 `LanePublishedMap` 使用 primitive map 的删除压紧行为保持固定活跃量下的底层存储复用；零准入序号不保存为独立元数据项。周转测试检查实际内部数组身份，避免遗漏插入/删除引起的反复整表重建。


在途批次上下文由对应 `PendingMatching` 环槽持有，不再维护第二份序号到批次的 Map。批次仅保留 Owner 顺序链接，终态时摘除并清空引用，再复用事件和上下文；延期推进仍直接选择队首。风险/撤单续步保留命令头身份，只在解码成功后更新 payload，不复制整份在途状态。

顺序现货批项结算改为所属 Lane 执行、完成后续项；批次订单元数据和提交序号通过现有 LaneCommitEvent 一次写入。Owner保留顺序、资金边界及终态发布；顺序项准入、改单冻结和原生拒绝解冻也已迁移永久Lane；Owner保留只读校验及pending索引回执。其它控制/单笔非流水路径仍待核对，不能宣称账户写入已全路径归属Lane。


Aeron 普通下单与流水线批量下单已采用 Matcher→Lane 直接结果发布，Owner 保留必要的有序提交；线程所有权、事件复用和持仓身份生命周期见 `surprising-aeron-core/README.md`。本机单成员验证与剩余Owner瓶颈以根目录 `PERFORMANCE_VALIDATION.md` 的最新交付汇总为准。
