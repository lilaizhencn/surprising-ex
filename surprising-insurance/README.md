...
Surprising Exchange insurance-fund module. It manages the perpetual insurance fund and covers explicit bankruptcy deficits created by account settlement.

## Modules

- `surprising-insurance-api`: insurance RPC contracts and long-based response models.
- `surprising-insurance-provider`: fund adjustment API, deficit coverage worker, fund ledger, and account-ledger integration.
...
- Fund ledger has a unique reference index to prevent duplicate admin adjustments or duplicate coverage ledger rows.
...
- Do not make insurance depend on external market data; it should consume finalized account deficits only.
