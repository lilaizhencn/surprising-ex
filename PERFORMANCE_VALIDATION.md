...
## 2026-09-09 188a10bd 云端直接压测（采集前锁定）
- 用户明确要求服务器直接压测，不执行小样本。surprising-ae591/asia-southeast1-b既有3Core+1load，4台n2-custom-8-16384，HotSpot Temurin25.0.4.1，真实私网UDP/日志复制；当前master188a10bd，service SHA096a4cd3d68c7c737f2f6db17760ef13043581a92db01d3b1e67ba1f148bb9a5、benchmarks SHA19c45b7eaef503046707fb327200a7967c255db7e32290ffc267ed224c70f537。先停止开机自启旧服务，清理上一轮已确认的临时目录，不动历史业务数据；本轮新目录/home/atomex/ex-gcp-188a10bd。
- 沿用失败轮交易场景：ClusterMixedCapacityMain，LINEAR_PERPETUAL/trading-stream=true/operational=false，1769用户/256symbol/batch20、4Lane/1matcher、global=session256、1命令+1保留查询session、seed111001，30秒预热+120秒测量。固定初始化模板及资金/持仓/冻结/订单断言，持续异步闭环，未修正CO，WS/Valkey及operational后台侧载不测。
- Core4GiB ZGC/AlwaysPreTouch/NMTsummary/SHARED_NETWORK/service YIELDING/Lane spin0，JFR profile maxsize128m；load2GiB ZGC/SHARED，无本地Core压测。每5秒读取/proc线程CPU差分（按单核%）、内存/磁盘，磁盘少于10GiB终止；JFR带采样吞吐用于诊断，未预设绝对吞吐门槛，不作旧版本A/B。
- 通过条件：完整测量及最终金融核对PASS，offered=terminal/unfinished0/fundsDiff0，无批次上下文冲突、节点失败、未知结果。分开报告ops/messages/fills及各类响应延迟；真实三节点持续吞吐尚不能替代完整故障恢复、全产品性能和长期泄漏验证。管理SSH启动只读重试，业务请求不重试未知结果、不提高30秒deadline。执行后收集摘要证据、清理本轮本地/tmp/ex-gcp-188a10bd与远端临时产物，finally停止四VM并核实TERMINATED。
- 实际部署已核对三个service哈希及load哈希与预锁一致，非旧包；直接执行正式负载，无小样本。mixedLossLifecycle=PASS（liquidation/insurance/adl），mixedSetup=PASS users1769/retail1000/symbols256/initialFunds1768000000125。连续交易预热阶段再次发生admitted请求ResultUnknownException，未到有效measurement、未完成最终资金与状态核对，没有可报告的吞吐或尾延迟。
- core-1成为leader后再次发生AgentTerminationException(snapshot projection batch is already active)，栈为OrderedCommitCoordinator.beginCommitPublicationBatch:1266 ← MatchingCommandAdmission.prepareMatching:230 ← beginMatching:129 ← TradingCoreRuntime.apply:1000/applyDecodedCommand:506/applyClusterCommand:495 ← SurprisingClusteredService.progressCommandsInScope:219/onSessionMessage:135。core-0从onTimerEvent/drainFromLogEvent同样失败；core-2接任leader但失去quorum，结束时highWaterMark53/pending31/windows7812/commands8595/dependencyFences1452/controlFences13。
- 结论：188a10bd确实修复了本地复现的提前恢复路径，但云端结果证明提交发布批次生命周期仍有未覆盖分支，不能宣称该生产故障已完全修复。仍不是历史数据（全新本轮目录）或算力饱和；未提高超时、重试未知交易或删除保护来继续测量。本轮仅测试，无新的生产代码修改。
...
## 2026-09-09 d1a1da96 真实三节点压测（采集前锁定）
- 用户要求开始服务器压测；仅当前 master d1a1da96，不做旧版本对照。surprising-ae591/asia-southeast1-b，既有3Core+1load，均n2-custom-8-16384；停止旧自启服务，在/home/atomex/ex-gcp-d1a1da96使用全新本轮目录。service SHA256 937ce4609458e9a1b9ef7c65f67fa2996033d388010766f8056294cbd6c8e8b6；benchmarks SHA256 05d6f2158d1f1eb14ff6fea0c85130ce5492ae30f36d9a34917c00bbfd63d968。
- 沿用上轮 ClusterMixedCapacityMain 连续交易场景：LINEAR_PERPETUAL、trading-stream=true、operational=false、1769用户/256symbol/batch20、4Lane/1matcher、global=session in-flight256、1命令+1保留查询session、seed111001；30秒预热+120秒测量，持续异步提交，初始化/测量边界排空及最终金融核对保持不变，不进行小样本先测。未修正CO，WS/Valkey与operational后台侧载不测。
- 真实私网UDP和日志复制。云端HotSpot JDK25（开机核实）；Core4GiB ZGC/AlwaysPreTouch/NMTsummary/SHARED_NETWORK/service YIELDING/Lane spin0；load2GiB ZGC/SHARED。JFR profile maxsize128MiB/dumponexit，各Core每5秒采集线程CPU、RSS、磁盘，磁盘小于10GiB停止本轮节点。业务deadline30秒，未知结果不重试。
- 通过条件：完整测量及最终金融/状态核对PASS，offered=terminal、unfinished0、资金差额0，无上下文冲突、节点失败、未知结果；分别报告terminal business ops/s、Core messages/s、fills/s及负载器提供的分类尾延迟。此轮诊断未设置绝对吞吐门槛，带JFR结果不代替无采样容量验收；不把启动CPU或等待自旋当作有效饱和。运行结束收集必要证据、清理本轮产物并停止四VM，核实TERMINATED。
- 实际四台HotSpot Temurin25.0.4.1，安装包哈希全部与预锁一致，core-0为leader。mixedLossLifecycle与mixedSetup均PASS，initialFunds1768000000125；正式测量开始epochMillis1788965540849，随后9个10秒窗口分别16317.681、15485.413、11942.446、10411.308、10393.008、10228.000、3500.200、0、0 terminal business ops/s。累计终态782819后不再推进，在途256，负载器最终ResultUnknownException退出1；未完成120秒测量和最终金融核对，因此无有效持续吞吐、最终Core messages/s、fills/s或分类尾延迟结论。
- 两个从节点均因replicated command backlog capacity exhausted退出，栈PendingClusterIngress.add:20 ← SurprisingClusteredService.processIngress:160/onSessionMessage:135。该保护同时检查8192条记录和64MiB payload，现有日志不能区分命中哪一个上限。两个从节点退出导致失去多数派；leader退出汇总highWaterMark64/pending64/windows61160/commands124112。未再观察到上轮snapshot projection batch上下文异常，但本轮仍失败，不宣称全部故障已解决。
- 5秒线程采样core0/1/2分别40/32/23条。测量开始至14:53:25UTC前，leader owner平均99.95%单核CPU、matcher6.76%、四Lane合计6.12%；两个从节点相同区间owner约99.95%/99.94%。leader JFR在14:52:20–14:53:20UTC有1725个owner Java/native采样，其中1291个（74.84%）为Thread.yield0 ← YieldingIdleStrategy.idle ← AgentRunner.doWork；停滞后14:53:25–14:54:00UTC为645/646（99.85%）。采样比例不是精确CPU时间占比；证据表明满核不能视作有效业务饱和，matcher/Lane没有饱和。
- 静态核对：SurprisingClusteredService.doBackgroundWork在pendingIngress或commandWindow非空时直接返回，交易续接依赖日志/定时器回调；新日志继续进入有界pendingIngress。从节点队列耗尽与此异步推进/背压边界相关，尚未凭失败从节点的完整状态/JFR确定积压形成的具体续接分支，不能仅扩大队列或增加自旋作为修复。本轮仅测试与分析，没有修改生产逻辑。
