...
- 用户最新要求（2026-09-09）：每轮测试分析结束后，先将必要的参数、结果、异常摘要和未验证范围写入现有验证记录，再停止本轮进程并清理本轮临时集群目录、Aeron Archive/录制文件、JFR、堆转储、临时日志及测试报告，不长期保留大体积测试产物。此要求覆盖下文历史原始 artifact 保留约定；记录中注明原始产物已清理，不能继续把已删除路径当作可访问证据。只删除已确认属于本轮的生成文件，不删除业务数据或用户文件。
- 测试前和运行中检查剩余磁盘空间，给日志及录制设置合理上限；出现空间不足时停止该轮并记录环境异常，不将该轮失败未经定位就归因于交易业务。
...
- 用户最新要求（2026-09-10）：本机验证与压测统一使用 Aeron Cluster 单节点开发模式，每次仅启动一个真实成员，保留网络、Archive 日志及交易 Core；不得在本机同时启动三个节点。此要求覆盖下文历史三节点本机规则。两方案对照依次运行，参数与统计口径一致；本机结果不作为三节点云端容量结论。

- 用户最新要求（2026-09-09晚）：释放 Google Cloud 压测实例、磁盘和 IP，先在本机推进；允许本机小规模压测和 JMH/JFR 定位瓶颈。此要求覆盖下文仅限真实三节点的执行地点限制；本机诊断仍须区分单进程与三进程网络链路，不作为云端容量验收，继续执行资金/状态核对和产物清理。

- 2026-09-08用户为当前饱和定位明确解除旧脚本限制：允许调整全局in-flight、session实际提交窗口、连接和发压业务编排，逐级加压寻找真实三节点吞吐平台。此任务不受下文历史固定256档位限制；每档仍须预锁实际参数/资金与状态校验，分别记录mixed与连续交易场景。不得放宽交易正确性、绕过真实三节点或用无业务自旋伪造95%计算饱和；结束后停止四VM，下一次明确要求测试才再开机。

- 用户最新要求（2026-09-07）：后续所有交易吞吐、并发、尾延迟压测及相关性能采样，必须在三个独立节点组成的真实 Aeron Cluster 环境中执行，使用真实网络、日志复制和交易 Core；不得再用本地单进程/内存直调或 mock Session 的性能基准替代或另行开展交易性能验证。本地仅用于构建、功能/正确性测试和已采集 artifact 的离线分析。下述 JMH/JFR 要求仍适用，但新的性能执行必须覆盖真实三节点链路；历史本地结果保留为历史记录，不作为后续压测依据。
- 用户进一步明确：吞吐压测器应持续异步发压，不得逐笔等待响应或等maker终态才发taker来限制发压；只保留全局256在途背压、金融规则要求的行情前置，以及初始化/测量边界排空和正确性核对。当前独立普通订单使用MATCH_STREAM（买卖GTC各半）；不得因用户说“不要等待”擅自移除Core的确定性/结算完成边界，也不要转成Core等待策略对照来替代直接吞吐测试。
...
- 后续每次性能验证的 in-flight 必须固定为 `256`；不得采集、补跑、横向比较或以 `64`等其他 in-flight 档位形成 smoke、基线或验收结论。历史其他档位数据只能作为历史背景，不能与新的 `256 in-flight` 结果直接比较。
