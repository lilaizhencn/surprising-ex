...
| `trading-trigger` | `http://localhost:9084/api/v1/trading/trigger-orders` | yes |
| `account` | `http://localhost:9086/api/v1/accounts` | yes |
| `risk` | `http://localhost:9088/api/v1/risk` | yes |
...
Core local admin lists use a common cursor paging contract. `/api/v1/admin/approvals`,
`/api/v1/admin/audit/login-logs`, and `/api/v1/admin/audit/operations` accept `limit`, `cursor`, and
`sort`, keep their original list fields, and additionally return `nextCursor`, `hasMore`, `sort`,
and `limit`. Approval lists support `requestedAt.desc` and `requestedAt.asc`; audit logs support
`createdAt.desc` and `createdAt.asc`.

Cross-table order timelines, market/trading operations metrics, account valuation, reconciliation,
and daily reporting are intentionally not implemented in the gateway. A future
`surprising-finance-ops` module must use its own datasource and physical database, then build query
projections from domain events, outbox events, or controlled CDC. It must not run report JOINs
against the primary trading database.

System monitoring endpoints live under `/api/v1/admin/system`: `/routes` returns public and admin route configuration, `/health` probes backend `/actuator/health`, and `/observability` aggregates Kafka consumer lag, WebSocket connection/subscription metrics, and backend `/actuator/prometheus` scrape status. These endpoints require `admin.system.read`. Kafka lag is disabled by default; enable it in production with `ADMIN_KAFKA_LAG_ENABLED=true` and `ADMIN_KAFKA_BOOTSTRAP_SERVERS`. The business-database-backed `/metrics` endpoint and local alert center were removed; future equivalents belong in the independent operations database or observability platform.

Downstream `trading-orders` and `trading-trigger` admin proxy routes keep bounded single-domain
detail paging for online support actions. Cross-domain timelines and aggregated operations reports
belong to the independent finance-operations database described above.
...
and returns `nextCursor`, `hasMore`, `sort`, and `limit`; fee writes remain sensitive and require approval.
Automatic tier qualification belongs to the future finance-operations system.

Risk, liquidation, and ADL admin single-table lists use the same `limit/cursor/sort` response contract.
`/api/v1/admin/gateway/risk-admin/liquidation-candidates` supports `eventTime.desc` and `eventTime.asc`;
`/api/v1/admin/gateway/liquidation-admin/orders` and
